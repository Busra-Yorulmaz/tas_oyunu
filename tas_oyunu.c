...
int main(){
	...
	//ikinci oyuncu taþlarý
   	...
   	for(k=0;k<2;k++){
    	...
    	int oyuncu = rand()%2; // oyuna başlayacak oyuncu random seçilir.
		int renk   = rand()%2;	//Başlangıçta verilecek taþýn mavi veya kırmızı olmasına rastgele karar verilir.
		int sayi   = 1+(rand()%10); //1-10 arası toplardan hangisinin verileceði rastgele seçilir
		int sec    = rand()%2; // iki uygun taş da varsa oynanacak taş random seçilir.
		int adim   = 1;  // hamle sayisini gösterir.
	...
	while(1==1){
		...
		// HER ADIMDA TAŞLAR VE ADIMLAR EKRANA BASTIRILIYOR
		printf("\n1.OYUNCU TAHTASI\n"); 
	   	for(i=0;i<2;i++){
 			if(i ==0)printf("M: ");	
			if(i==1) printf("K: ");
